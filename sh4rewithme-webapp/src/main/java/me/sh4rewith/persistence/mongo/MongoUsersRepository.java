...
import static me.sh4rewith.persistence.mongo.mappers.UserInfoMapper.USER_INFO_COLLECTION;
...
public class MongoUsersRepository implements UsersRepository {
	...
	public void store(UserInfo userInfo) {
		...
		mongo.upsert(userQuery, new Update().set(MONGO_DOC_ID, userInfo.getId()), USER_INFO_COLLECTION);
		mongo.upsert(userQuery, new Update().set(EMAIL, userInfo.getEmail()), USER_INFO_COLLECTION);
		mongo.upsert(userQuery, new Update().set(FIRST_NAME, userInfo.getFirstname()), USER_INFO_COLLECTION);
		mongo.upsert(userQuery, new Update().set(LAST_NAME, userInfo.getLastname()), USER_INFO_COLLECTION);
		mongo.upsert(userQuery, new Update().set(CREDENTIALS, userInfo.getCredentials()), USER_INFO_COLLECTION);
		mongo.upsert(userQuery, new Update().set(REGISTRATION_STATUS, userInfo.getRegistrationStatus()), USER_INFO_COLLECTION);
		mongo.upsert(userQuery, new Update().set(HASH, userInfo.getUserHash()), USER_INFO_COLLECTION);
	}

	@Override
	public UserInfo getUserInfoById(String userId) {
		UserInfoMapper userInfoMapper = new UserInfoMapper();
		mongo.executeQuery(new Query(Criteria.where(MONGO_DOC_ID).is(userId)), USER_INFO_COLLECTION, userInfoMapper);
	...
	public List<UserInfo> findAllUserInfoBut(String userId) {
		UserInfoMapper userInfoMapper = new UserInfoMapper();
		mongo.executeQuery(new Query(Criteria.where(MONGO_DOC_ID).ne(userId)), USER_INFO_COLLECTION, userInfoMapper);
		return userInfoMapper.getUserInfoList();
	}

	@Override
	public Long countAll() {
		return mongo.count(new Query(Criteria.where(MONGO_DOC_ID).regex(".*")), USER_INFO_COLLECTION);
	}

	@Override
	public UserInfo getUserInfoByHash(String userHash) {
		UserInfoMapper userInfoMapper = new UserInfoMapper();
		mongo.executeQuery(new Query(Criteria.where(HASH).is(userHash)), USER_INFO_COLLECTION, userInfoMapper);
		return userInfoMapper.getUserInfo();
	}
	...
	public void changeRegistrationStatusByHash(String userInfoHash, RegistrationStatus registrationStatus) {
		...
		mongo.upsert(userQuery, new Update().set(REGISTRATION_STATUS, registrationStatus), USER_INFO_COLLECTION);
	}
}
